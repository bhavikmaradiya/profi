      ...
      children: [
        Expanded(
          child: BlocBuilder<FirebaseFetchProjectsBloc,
            ...
            buildWhen: (previous, current) =>
                previous != current && current is FirebaseBDMInfoChangedState,
            builder: (context, state) {
              final profileInfo = addProjectFieldBloc.getCurrentUserRole();
              List<DropDownModel> list = firebaseProjectsBlocProvider!.bdmList
                  .mapIndexed(
                    (i, bdm) => DropDownModel(
                      id: i,
                      uniqueId: bdm.userId,
              ...
              final selectedBdm = addProjectFieldBloc.getSelectedBDM();
              String? selectedId = (selectedBdm ?? profileInfo?.userId);
              DropDownModel? selected;
              if (selectedId != null) {
                selected = list.firstWhereOrNull(
                  (element) => selectedId == element.uniqueId,
                );
              }
              final isDisableDropDown = profileInfo != null &&
                  profileInfo.role == UserRoleEnum.bdm.name;
              if (isDisableDropDown &&
                  selectedId == profileInfo.userId &&
                  selected != null &&
                  (selectedBdm == null || selectedBdm != profileInfo.userId)) {
                addProjectFieldBloc.add(
                  BdmInfoSelectionChangeEvent(selected.uniqueId),
                );
                //remove if any change comes from db in already selected user
              } else if (selectedBdm != null && selected == null) {
                addProjectFieldBloc.add(
                  BdmInfoSelectionChangeEvent(null),
                );
              }
        ...
        Expanded(
          child: BlocBuilder<FirebaseFetchProjectsBloc,
              FirebaseFetchProjectsState>(
            buildWhen: (previous, current) =>
                previous != current && current is FirebasePMInfoChangedState,
            builder: (context, state) {
              final profileInfo = addProjectFieldBloc.getCurrentUserRole();
              List<DropDownModel> list = firebaseProjectsBlocProvider!.pmList
                  .mapIndexed(
                    (i, pm) => DropDownModel(
                      id: i,
                      uniqueId: pm.userId,
                  ...
                  .toList();
              list.sort((a, b) => a.value.compareTo(b.value));
              final selectedPM = addProjectFieldBloc.getSelectedPM();
              String? selectedId = (selectedPM ?? profileInfo?.userId);
              DropDownModel? selected;
              if (selectedId != null) {
                selected = list.firstWhereOrNull(
                  (element) => selectedId == element.uniqueId,
              ...
              if (isDisableDropDown &&
                  selectedId == profileInfo.userId &&
                  selected != null &&
                  (selectedPM == null || selectedPM != profileInfo.userId)) {
                addProjectFieldBloc.add(
                  ProjectManagerInfoSelectionChangeEvent(selected.uniqueId),
                );
                //remove if any change comes from db in already selected user
              } else if (selectedPM != null && selected == null) {
                addProjectFieldBloc.add(
                  ProjectManagerInfoSelectionChangeEvent(null),
                );
              }
              return AppDropDownField(
