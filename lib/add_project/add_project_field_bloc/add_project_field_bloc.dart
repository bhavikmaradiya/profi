...
class AddProjectFieldBloc
  ...
  final List<DropDownModel> _bdmList = [];
  final List<DropDownModel> _pmList = [];
  ...
  AddProjectFieldBloc() : super(AddProjectFieldInitialState()) {
    ...
    on<GenerateBDMListEvent>(_generateBDMList);
    on<GeneratePMListEvent>(_generatePMList);
    ...
    add(GenerateBDMListEvent());
    add(GeneratePMListEvent());
    add(
      ...
      for (int i = 0; i < docs.length; i++) {
        try {
          ...
          _bdmList.add(
            DropDownModel(
              id: i,
              uniqueId: profileInfo.userId,
              value: profileInfo.name ?? '',
            ),
          );
        } on Exception catch (_) {}
      }
      _bdmList.sort((a, b) => a.value.compareTo(b.value));

      DropDownModel? defaultSelected;
      // set current user as default BDM if roles are same!
      if (_profileInfo != null && _profileInfo?.role == UserRoleEnum.bdm.name) {
        defaultSelected = getDefaultBDM();
        if (defaultSelected != null) {
          _selectedBdmUserId = defaultSelected.uniqueId;
        }
      } else {
        defaultSelected = getSelectedBDM();
      }
      emit(
  ...
  _generatePMList(
    ...
    if (docs.isNotEmpty) {
      ...
      _pmList.sort((a, b) => a.value.compareTo(b.value));

      DropDownModel? defaultSelected;
      // set current user as default PM if roles are same!
      if (_profileInfo != null &&
          _profileInfo?.role == UserRoleEnum.projectManager.name) {
        defaultSelected = getDefaultPM();
        if (defaultSelected != null) {
          _selectedPmUserId = defaultSelected.uniqueId;
        }
      } else {
        defaultSelected = getSelectedPM();
      }
      emit(
  ...
  _onBdmSelectionChanged(
    ...
    _selectedBdmUserId = event.bdmInfo.uniqueId;
  }
  ...
  _onProjectManagerInfoChanged(
    ...
    _selectedPmUserId = event.pmInfo.uniqueId;
  }
...
// 4. milestone is adding by user by clicking on add button
    ...
    if (isNeedToAutoGenerate) {
      ...
      if (!fromAddNewMilestoneButton && _milestones.isNotEmpty &&
  ...
  DropDownModel? getSelectedBDM() {
    if (_selectedBdmUserId != null) {
      final index = _bdmList.indexWhere(
        (element) => element.uniqueId == _selectedBdmUserId,
      );
      if (index != -1) {
        return _bdmList[index];
      }
    }
    return null;
  }

  DropDownModel? getDefaultBDM() {
    final index = _bdmList.indexWhere(
      (element) => element.uniqueId == _profileInfo?.userId,
    );
    if (index != -1) {
      return _bdmList[index];
    }
  ...
  DropDownModel? getSelectedPM() {
    if (_selectedPmUserId != null) {
      final index = _pmList.indexWhere(
        (element) => element.uniqueId == _selectedPmUserId,
      );
      if (index != -1) {
        return _pmList[index];
      }
    }
    return null;
  }

  DropDownModel? getDefaultPM() {
    final index = _pmList.indexWhere(
      (element) => element.uniqueId == _profileInfo?.userId,
    );
    if (index != -1) {
  ...
  List<DropDownModel> getBdmList() {
    return _bdmList;
  }

  List<DropDownModel> getPmList() {
    return _pmList;
  }
