...
class FirebaseAddProjectBloc
  ...
  _onFirebaseEditProjectSaveEvent(
    ...
    await batch.commit();
    await _deleteTransactionOnMilestoneDelete(removedMilestoneList);
  ...
  _deleteTransactionOnMilestoneDelete(List<MilestoneInfo> milestones) async {
    final List<String> milestoneToDelete = [];
    for (int k = 0; k < milestones.length; k++) {
      final milestoneId = milestones[k].milestoneId;
      if (milestoneId != null) {
        final querySnapshot = await _fireStoreInstance
            .collection(FireStoreConfig.transactionsCollection)
            .where(
        ...
        if (documents.isNotEmpty) {
          for (int i = 0; i < documents.length; i++) {
            milestoneToDelete.add(documents[i].id);
          }
        }
      }
    }

    if (milestoneToDelete.isNotEmpty) {
      final batch = _fireStoreInstance.batch();
      for (int j = 0; j < milestoneToDelete.length; j++) {
        final ref = _fireStoreInstance
            .collection(FireStoreConfig.transactionsCollection)
            .doc(milestoneToDelete[j]);
        batch.delete(ref);
      }
      await batch.commit();
    }
