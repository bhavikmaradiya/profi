...
import '../dialog/wallet_operation_dialog.dart';
...
class ShowDialogUtils {
  ...
  static _onSingleMilestonePaidOperation({
    ...
    double paidAmount = double.parse(amount.trim());
    BlocProvider.of<MilestoneOperationsBloc>(
      context,
      listen: false,
    ).add(
  ...
  static _onSingleMilestoneUnPaidOperation({
    ...
    final currentTime = DateTime.now().millisecondsSinceEpoch;
    final unPaidAmount = double.parse(amount);
    BlocProvider.of<MilestoneOperationsBloc>(
      context,
      listen: false,
    ).add(
  ...
  static _onMultipleMilestoneUnPaidOperation({
    ...
    if (multipleMilestoneInfo.isNotEmpty) {
      ...
      double unPaidAmount = double.parse(amount.trim());
      BlocProvider.of<WalletBloc>(
        context,
        listen: false,
      ).add(
  ...
  static showWalletOperationDialog({
    required BuildContext context,
    required WalletInfo walletInfo,
    required WalletEnums walletType,
  }) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) {
        return Dialog(
          ...
          child: WalletOperationDialog(
            whichWallet: walletType,
