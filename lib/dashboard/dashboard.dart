...
class _DashboardState extends State<Dashboard>
  ...
  void didChangeDependencies() {
    ...
    _walletBloc ??= BlocProvider.of<WalletBloc>(
      context,
      listen: false,
    );
    ...
    if (AppUtils.isUserLoginAfterLogOut) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        ...
        _transactionBloc?.add(ListenTransactionChangesEvent());
        _walletBloc?.add(ListenWalletInfoChangesEvent());
      });
    }
      ...
      child: DefaultTabController(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: BlocBuilder<FirebaseFetchProjectsBloc,
                FirebaseFetchProjectsState>(
              buildWhen: (prev, current) =>
                  prev != current &&
                  (current is ProjectSearchInitializedState ||
                      current is ProjectSearchClosedState),
              builder: (context, state) {
                if (state is ProjectSearchInitializedState) {
                  return AppSearchView(
                    onTextChange: (searchBy) {
                      _firebaseFetchProjectsBloc?.add(
                        ProjectSearchTextChangedEvent(searchBy),
                      );
                    },
                    onCloseSearch: () {
                      _firebaseFetchProjectsBloc
                          ?.add(ProjectSearchClosedEvent());
                    },
                  );
                }
                return Text(
              ...
              BlocBuilder<FirebaseFetchProjectsBloc,
                ...
                builder: (context, state) {
                  ...
                  return InkWell(
                    onTap: () {
                      _firebaseFetchProjectsBloc
                          ?.add(ProjectSearchInitializeEvent());
                    },
                    borderRadius: BorderRadius.circular(
