...
class FirebaseAuthBloc extends Bloc<FirebaseAuthEvent, FirebaseAuthState> {
  ...
  _fetchFirebaseProfileInfo(
    ...
    if (userCredentials.user != null) {
      ...
      if (profileInfo != null) {
        ...
        await _updateWalletInfo(firebaseUserId);
  ...
  _updateWalletInfo(String firebaseUserId) async {
    final walletRef = FirebaseFirestore.instance
        .collection(FireStoreConfig.walletCollection)
        .doc(firebaseUserId);
    final document = await walletRef.get();
    WalletInfo? walletInfo;
    try {
      if (document.data() != null) {
        walletInfo = WalletInfo.fromSnapshot(document);
      }
    ...
    walletInfo.updatedAt ??= DateTime.now().millisecondsSinceEpoch;
    walletRef.set(walletInfo.toMap());
  }
